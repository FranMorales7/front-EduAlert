...
export default function TablaSalidas() {
  ...
  const handleEliminar = (id) => {
    ...
    axios
      ...
      .then(() => {
        // Actualizar el estado de incidencias eliminando la salida eliminada
  ...
  const abrirModalEditar = (salida) => {
    setEditingTrip(salida); // carga datos en formulario
    setIsModalOpen(true);
  }
  ...
  return (
    <div className="p-6 bg-white shadow-xl rounded-xl">
      ...
      <div className='max-h-[500px] overflow-auto rounded border border-gray-300'>
        ...
        {/* Modal */}
        <Dialog open={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingTrip(null); }} className="relative z-50">
          <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <Dialog.Panel className="w-full max-w-md bg-white p-6 rounded shadow">
              <Dialog.Title className="text-lg font-semibold mb-4">
                {editingTrip ? 'Editar Incidencia' : 'Nueva Incidencia'}
              </Dialog.Title>
              <FormularioAviso 
